...
/**
* Map Connec Resource representation to/from SugarCRM Model
* You need to extend this class an implement the following methods:
* - getId($model) Returns the SugarCRM entity local id
* - loadModelById($local_id) Loads the SugarCRM entity by its id
* - mapConnecResourceToModel($resource_hash, $model) Maps the Connec resource to the SugarCRM entity
* - mapModelToConnecResource($model) Maps the SugarCRM entity into a Connec resource
* - persistLocalModel($model) Saves the SugarCRM entity
* - matchLocalModel($resource_hash) (Optional) Returns an SugarCRM entity matched by attributes
*/
abstract class BaseMapper {
  ...
  // Overwrite me!
  // Map the Connec resource attributes onto the SugarCRM model
  abstract protected function mapConnecResourceToModel($resource_hash, $model);

  // Overwrite me!
  // Map the SugarCRM model to a Connec resource hash
  abstract protected function mapModelToConnecResource($model);

  // Overwrite me!
  // Persist the SugarCRM model
  ...
  // Persist a list of Connec Resources as SugarCRM Models
  ...
  // Find an SugarCRM entity matching the Connec resource or initialize a new one
  ...
  // Transform an SugarCRM Model into a Connec Resource and push it to Connec
